...
# Selected digits to be used.
DIGITS = [0, 1]

# One hot representation.
OUTPUT_DIMS = len(DIGITS)

MODEL_FILE = '../data-repo/models/mnist-subset.pkl'
...
class Model(object):
    def __init__(self, hyperparams):
        ...
        result = make_model(hyperparams, INPUT_DIMS, OUTPUT_DIMS,
    ...
    def save_image(self, image_flattened, out_file):
        image_dim = int(math.sqrt(image_flattened.shape[0]))
        image = image_flattened.reshape(image_dim, image_dim)
        fig = plt.figure()
        ax1 = fig.add_subplot(121)
    ...
    def translate(self, test_set, original_test_set):
        print_weight_counts(self.mlp)
        num_examples = 100
        examples_indices = np.arange(num_examples)
        examples_X = test_set.X[examples_indices]
        original_X = original_test_set.X[examples_indices]
        examples_Y = test_set.y[examples_indices]

        # for i in range(num_examples):
        #     self.save_image(examples_X[i],
        #                     '../data-repo/results/MNIST-input-resized_{}'.format(i))
        #     self.save_image(original_X[i],
        #                     '../data-repo/results/MNIST-input-original_{}'.format(i))
        ...
        results_dir = os.path.join(SIMULATION_RES_DIR, 'mnist-subset')

        with open('../data-repo/mathematica/mnist-subset.wls', 'w') as f:
    ...
    def filter_digits(self, data):
        labels = data.y.flatten()
        mask = [False] * len(labels)
        for digit in DIGITS:
            mask = np.logical_or(mask, labels == digit)
        for i, digit in enumerate(DIGITS):
            # Reset labels to be in range 0 .. len(DIGITS)
            indices = np.where(labels == digit)
            labels[indices] = i
        indices = np.where(mask)
        data.y = np.expand_dims(labels[indices], axis=1)
        data.X = data.X[indices]
        return data

    def load_dataset(self):
        # TODO: Check if need to do centering (original BinaryConnect does).
        center_images = True
        ...
        original_test_set = copy.copy(test_set)

        train_set = self.filter_digits(train_set)
        valid_set = self.filter_digits(valid_set)
        test_set = self.filter_digits(test_set)
        ...
        return train_set, valid_set, test_set, original_test_set

    def train(self, save_files=True):
        train_set, valid_set, test_set, original_test_set = self.load_dataset()
...
if __name__ == "__main__":
    ...
    if args.translate:
        ...
        train_set, valid_set, test_set, original_test_set = model.load_dataset()
        model.translate(test_set, original_test_set)
